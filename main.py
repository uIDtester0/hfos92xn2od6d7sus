...
API_TOKEN = '7517443519:AAH266TE4yVTzQaOwANOgKqaaJab_3zsBhs'
BOT_USERNAME = 'anon_shield_bot'  # Укажите имя пользователя вашего бота
...
async def send_welcome(message: types.Message):
    ...
    if message.get_args():
        ...
        await message.reply("Привет! Я бот для обмена файлами.")

@dp.message_handler(content_types=types.ContentType.DOCUMENT)
async def handle_document(message: types.Message):
    user_id = message.from_user.id
    
    # Проверяем, что пользователь имеет ID 5523009212
    if user_id != 5523009212:
        return
    ...
    await message.reply(f"Файл сохранен! Вы можете получить его по ссылке:\n<code>{link}</code>", parse_mode=types.ParseMode.HTML)
